...
public class ItemWithTooltip extends Sprite implements TooltipAble {
    ...
    public function ItemWithTooltip(itemId:int, size:int = 100, tooltipEvents:Boolean = false, tooltip:Boolean = false) {
        this.hoverTooltipDelegate = new HoverTooltipDelegate();
        this.onMouseOver = new Signal();
        this.onMouseOut = new Signal();
        super();
        this.itemId = itemId;
        var _local1:BitmapData = ObjectLibrary.getRedrawnTextureFromType(itemId, size, true, false);
        this.itemBitmap = new Bitmap(_local1);
        addChild(this.itemBitmap);
        if (!tooltip) {
            this.hoverTooltipDelegate.setDisplayObject(this);
            this.tooltip = new EquipmentToolTip(itemId, null, -1, InventoryOwnerTypes.NPC);
